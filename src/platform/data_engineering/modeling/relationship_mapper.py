...
Usage:
    from src.platform.data_engineering.modeling.relationship_mapper import generate_relationship_report
...
def validate_primary_keys(
    ...
    null_counts = {col: df[col].null_count() for col in available_keys}
...
def validate_referential_integrity(
    ...
    if parent_key not in parent_df.columns:
        return {
            ...
            "error": f"Parent key '{parent_key}' not found",
        }
    if child_key not in child_df.columns:
        return {
            "relationship": relationship_name,
            "error": f"Child key '{child_key}' not found",
        }
...
def _compute_join_statistics(
    ...
    if "listings" in dataframes and "reviews" in dataframes:
        ...
        if "listing_id" in reviews_df.columns:
            reviews_per_listing = (
                reviews_df.group_by("listing_id").len().rename({"len": "review_count"})
            )
