...
def infer_schema(filepath: str | Path) -> dict[str, Any]:
    ...
    for col_name in df.columns:
        ...
        sample_values = (
            non_null.head(MAX_SAMPLE_VALUES).to_list() if non_null.len() > 0 else []
        )
...
def compute_column_stats(df: pl.DataFrame, col_name: str) -> dict[str, Any]:
    ...
    stats: dict[str, Any] = {
        ...
        "unique_pct": (
            round(non_null.n_unique() / non_null_count * 100, 2)
            if non_null_count > 0
            else 0.0
        ),
    }
...
def _compute_string_stats(col: pl.Series) -> dict[str, Any]:
    ...
    top_values = {
        str(row[value_col]): int(row[count_col])
        for row in top_entries.iter_rows(named=True)
    }
    ...
    return {
        ...
        "avg_length": (
            round(float(lengths.mean()), 1) if lengths.mean() is not None else None
        ),
...
def detect_outliers_iqr(
    ...
    examples = sorted(
        [_make_serializable(v) for v in outlier_values.head(10).to_list()]
    )
    ...
    return {
        ...
        "outlier_pct": (
            round(float(outlier_count) / total * 100, 2) if total > 0 else 0.0
        ),
...
def generate_data_quality_report(city_name: str) -> dict[str, Any]:
    ...
    for filepath in data_files:
        ...
        try:
            ...
            high_null_cols = [
                ...
                if c["null_pct"] > 50
                and c["column"] in ("price", "latitude", "longitude", "id")
            ]
            if high_null_cols:
    ...
    for _name, fr in file_reports.items():
        ...
        if completeness_list:
            avg_fill = sum(c["fill_rate_pct"] for c in completeness_list) / len(
                completeness_list
            )
            quality_scores.append(avg_fill)

    overall_score = (
        round(sum(quality_scores) / len(quality_scores), 1) if quality_scores else 0.0
    )
    ...
    logger.info(
        "Quality report saved: %s (score: %.1f)", output_path.name, overall_score
    )
    return report
