...
def _compute_derived_fields(
    df: pl.DataFrame, scrape_date: date | None
...
def _add_city_metadata(
    df: pl.DataFrame, city_name: str, city_config: dict[str, Any]
) -> pl.DataFrame:
    ...
    for df in dataframes:
        for name, dtype in zip(df.columns, df.dtypes):
            if name not in schema:
                ...
                if dtype in (pl.Utf8, pl.String) or schema[name] in (
                    pl.Utf8,
                    pl.String,
                ):
                    schema[name] = pl.String() if hasattr(pl, "String") else pl.Utf8()
                elif dtype in (pl.Float64, pl.Float32) or schema[name] in (
                    pl.Float64,
                    pl.Float32,
                ):
                    schema[name] = pl.Float64()
                elif dtype in (pl.Int64, pl.Int32) or schema[name] in (
                    pl.Int64,
                    pl.Int32,
                ):
