...
def _cast_boolean_columns(df: pl.DataFrame, columns: list[str]) -> pl.DataFrame:
    ...
    for col in columns:
        if col not in df.columns:
            continue
        exprs.append(
            pl.when(pl.col(col) == "t").then(True)
            .when(pl.col(col) == "f").then(False)
            .otherwise(None)
            .alias(col)
        )
...
def _count_amenities(df: pl.DataFrame) -> pl.DataFrame:
    ...
    return df.with_columns(
        ...
        .then(
            pl.col("amenities").str.count_matches(r'","').cast(pl.Int32) + 1
        )
...
def _parse_host_verifications_column(df: pl.DataFrame) -> pl.DataFrame:
    ...
    return df.with_columns(
        ...
        .then(
            pl.col("host_verifications").str.count_matches(r"'[^']+'").cast(pl.Int32)
        )
        .otherwise(pl.lit(0).cast(pl.Int32))
    ...
    for col_name, spec in strategies.items():
        ...
        if strategy in ("reject", "explicit_null"):
            ...
            logger.info(
                ...
                col_name, available_group_cols, null_remaining,
            )
...
def _compute_validation_flags(
    ...
    df = df.with_columns(
        (pl.col("_validation_flags") == "").alias("_is_valid")
    )
...
def _partition_and_save(
    ...
    logger.info(
        "Staging written: %s (%d rows)", output_path.name, valid_count
    )
    ...
    if rejected_count > 0:
        ...
        logger.warning(
            ...
            file_type, rejected_count, rejected_path.name,
        )
...
def _read_raw_file(city_name: str, file_type: str) -> tuple[Path, pl.DataFrame]:
    ...
    csv_files = [f for f in candidates if f.name.endswith(".csv") and not f.name.endswith(".csv.gz")]

    filepath = gz_files[0] if gz_files else (csv_files[0] if csv_files else None)
    if filepath is None:
        raise FileNotFoundError(
            f"No {file_type} file found in {raw_dir}"
        )
...
def clean_listings(city_name: str) -> CleaningResult:
    ...
    logger.info(
        ...
        input_rows, valid_count, rejected_count,
    )
...
def clean_city(city_name: str) -> dict[str, CleaningResult]:
    ...
    logger.info(
        ...
        city_name, total_input, total_output, total_rejected,
    )
    logger.info("=" * 60)
...
def _save_cleaning_summary(
    ...
    summary = {
        ...
        "files": {
            file_type: asdict(result) for file_type, result in results.items()
        },
        "totals": {
