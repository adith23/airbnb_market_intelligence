...
Usage:
    from pipeline.downloader import download_city
...
def _download_file(
    ...
    if dest_path.exists() and not force:
        ...
        logger.info("Already exists, skipping: %s (%s bytes)", dest_path.name, result["size_bytes"])
        return result
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            logger.warning(
                ...
                attempt, MAX_RETRIES, dest_path.name, exc,
            )
...
def verify_downloads(city_name: str) -> dict[str, Any]:
    ...
    logger.info(
        ...
        city_name, report["all_present"], report["all_readable"],
    )
    return report
