...
def _compute_regression_metrics(
    ...
    nonzero = y_t > 1.0
    mape = float(np.mean(np.abs(y_t[nonzero] - y_p[nonzero]) / y_t[nonzero]) * 100) if nonzero.sum() > 0 else np.nan
            ...
            try:
                ...
                results.append(LONGOResult(
                    ...
                    mean_residual=float(np.mean(
                        np.expm1(y_test_fold.values) - np.expm1(y_pred)
                    )) if log_transformed else float(np.mean(y_test_fold.values - y_pred)),
...
def cross_city_transfer(
    ...
    transfer_cols = [
        ...
        if not any(c.startswith(pat) for pat in exclude_patterns)
    ]
    ...
    for city in cities:
        ...
        metrics = _compute_regression_metrics(city_y.iloc[split_idx:].values, y_pred, log_transformed)
...
def neighbourhood_feature_ablation(
    ...
    logger.info(
        ...
        mae_with, mae_without, result["relative_degradation_pct"],
    )
    return result
...
def save_bias_report(report: BiasAuditReport) -> Path:
    ...
    json_data = {
        ...
        "longo_results": [
            {
                "city": r.city, "neighbourhood_group": r.neighbourhood_group,
                "n_test": r.n_test, "mae": round(r.mae, 2),
        ...
        "transfer_matrix": [
            {
                "train_city": t.train_city, "eval_city": t.eval_city,
                "n_train": t.n_train, "n_eval": t.n_eval,
                "mae": round(t.mae, 2), "mape": round(t.mape, 2) if not np.isnan(t.mape) else None,
                "r2": round(t.r2, 4), "mae_degradation": round(t.mae_degradation, 2),
            }
            for t in report.transfer_matrix
        ],
        "group_biases": [
            {
                "dimension": b.dimension, "group": b.group, "n": b.n,
                "mean_residual": b.mean_residual, "ci": [b.ci_lower, b.ci_upper],
                "significant": b.is_significant, "cohens_d": b.cohens_d,
                "magnitude": b.cohens_d_magnitude, "direction": b.direction,
            }
...
def _generate_bias_markdown(report: BiasAuditReport, output_dir: Path) -> None:
    ...
    if report.transfer_matrix:
        lines.extend(["", "## Cross-City Transfer Matrix\n",
    ...
    if report.longo_results:
        lines.extend(["", "## Neighbourhood Generalisation (LONGO)\n",
    ...
    if sig_biases:
        lines.extend(["", "## Significant Group Biases\n",
    ...
    if report.feature_ablation:
        ...
        lines.append(f"- MAE with neighbourhood features: {abl.get('mae_with_neighbourhood', 'N/A')}")
...
def run_bias_audit(
    ...
    longo = cross_neighbourhood_cv(
        best_model, feature_set.X, feature_set.y,
        feature_set.metadata_columns, config, baseline_mae,
    )

    # 2. Cross-city transfer
    transfer = cross_city_transfer(
        best_model, feature_set.X, feature_set.y,
        feature_set.metadata_columns, config,
    )

    # 3. Group-level bias on test set
    y_pred_test = best_trained.predict(split.X_test)
    biases = compute_group_bias(
        split.y_test.values, y_pred_test, split.meta_test, config,
    )

    # 4. Feature ablation
    ablation = neighbourhood_feature_ablation(
        best_model, feature_set.X, feature_set.y, config,
    )
