...
def _detect_model_type(model: Any) -> str:
    ...
    tree_indicators = (
        "forest", "xgb", "lgbm", "lightgbm", "gradient", "tree", "extra",
    )
...
def compute_shap_values(
    ...
    logger.info(
        ...
        model_type, len(X_sample), X_sample.shape[1],
    )
...
def local_explanations(
    ...
    for idx in sorted(indices)[:n]:
        ...
        top_pos = [
            ...
            for fi in sorted_feat_idx[::-1][:5] if sv[fi] > 0
        ]
        top_neg = [
            {"feature": feature_names[fi], "shap_value": round(float(sv[fi]), 4)}
            for fi in sorted_feat_idx[:5] if sv[fi] < 0
        ]
...
def explain_model(
    ...
    y_true_sample = split.y_test.iloc[sample_indices].values if hasattr(split.y_test, 'iloc') else None

    # Match metadata to the subsample
    meta_sample = split.meta_test.iloc[sample_indices] if hasattr(split.meta_test, 'iloc') else split.meta_test
    ...
    local_exp = local_explanations(
        shap_vals, X_sample, y_true_sample, y_pred_sample,
        expected_val, n_local, log_transformed
    )
