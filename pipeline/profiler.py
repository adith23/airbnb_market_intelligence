...
def infer_schema(filepath: str | Path) -> dict[str, Any]:
    ...
    for col_name in df.columns:
        ...
        sample_values = (
            non_null.head(MAX_SAMPLE_VALUES).to_list()
            if non_null.len() > 0
            else []
        )
    ...
    logger.info(
        ...
        filepath.name, row_count, col_count,
    )
    return schema
...
def compute_column_stats(df: pl.DataFrame, col_name: str) -> dict[str, Any]:
    ...
    stats: dict[str, Any] = {
        ...
        "unique_pct": round(
            non_null.n_unique() / non_null_count * 100, 2
...
def profile_file(filepath: str | Path) -> dict[str, Any]:
    ...
    logger.info(
        ...
        filepath.name, profile["row_count"], profile["column_count"],
    )
    return profile
...
def profile_city(city_name: str) -> dict[str, dict[str, Any]]:
    ...
    logger.info(
        "Profiling %d files for city: %s", len(data_files), city_name
    )
        ...
        if col_name in df.columns and df[col_name].dtype.is_numeric():
            ...
            if result.get("outlier_count", 0) > 0:
                logger.info(
                    ...
                    file_type, col_name,
                    result["outlier_count"], result["outlier_pct"],
                )
