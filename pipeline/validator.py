...
def validate_column(
    ...
    if regex_pattern and (series.dtype == pl.Utf8 or series.dtype == pl.String):
        try:
            non_matching = non_null.filter(
                ~non_null.str.contains(regex_pattern)
            ).len()
            if non_matching > 0:
                results["violations"].append({
                    "rule": "regex",
    ...
    if enum_values and (series.dtype == pl.Utf8 or series.dtype == pl.String):
        ...
        if outside > 0:
            ...
            unexpected = (
                non_null.filter(~non_null.is_in(enum_values))
                .unique()
                .head(5)
                .to_list()
            )
...
def detect_scraping_artifacts(df: pl.DataFrame) -> dict[str, Any]:
    ...
    for col_name in string_cols:
        ...
        encoding_count = col.filter(
            col.str.contains(r"Ã©|Ã¨|Ã |Ã§|â€™|â€œ|â€")
        ).len()
        if encoding_count > 0:
...
def generate_quality_report(city_name: str) -> dict[str, Any]:
    ...
    for filepath in data_files:
        ...
        passed_rules = sum(
            1 for v in column_validations.values()
