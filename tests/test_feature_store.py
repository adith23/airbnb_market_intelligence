...
def test_extract_amenity_flags():
    amenities = pd.Series(['["Wifi", "Kitchen", "Air conditioning"]', '["Pool", "TV"]', "[]", None])
...
def test_compute_distance_to_centre():
    ...
    city_centres = {"paris": {"lat": 48.8566, "lon": 2.3522}}
...
def test_create_interaction_terms():
    df = pd.DataFrame({"accommodates": [2, 4], "bedrooms": [1, 2]})
...
def test_handle_missing_values():
    ...
    config = {"add_indicators": True, "categorical": "__MISSING__"}
...
def test_encode_categoricals():
    df = pd.DataFrame({"color": ["red", "blue", "green", "red", "yellow", "purple", "orange"]})
